...
| **视频混剪** | 随机素材混剪 / 文件夹音频匹配混剪（支持断点续渲染）+ 参数模板 + 素材语义搜索 | 素材搜索首次需下载 CLIP 模型 |
| **AI 剪辑** | 文案驱动 + LLM 关键词 + CLIP 语义匹配画面，自动切镜合成 | LLM 需联网 |
| **AI 切片剪辑** | 长视频镜头检测 + AI 精彩度评估，自动拆分成多条短视频 | 否 |
| **文本精剪** | 转写即轨道：点着字剪视频、清理口头禅、对话式改片、非破坏性导出 | 首次需下载 Whisper 模型 |
| **自动流水线** | 素材分割 → 随机混剪 → 自动发布串成链，一键执行、失败即停、定时自动跑 | 否 |
| **热点选题** | 聚合百度/微博/抖音热榜，LLM 结合素材库生成选题与口播脚本 | 是 |
| **影视解说克隆** | 复刻参考视频的镜头节奏，替换画面 + 自定义配音字幕 | TTS 需联网 |
| **语音克隆** | GPT-SoVITS 音色克隆与合成（合成失败自动降级 Edge-TTS） | 否 |
| **自动发布** | 抖音 / 快手 / 小红书 / B站 / 微信视频号，批量 + 定时发布 | 是 |
| **矩阵对比** | 平台分组管理、近 7/30 天播放/互动对比、新作品推荐分组 | 否 |
...
- 🤖 **语义找素材**：输入「海边日落空镜」这类描述即可搜出最相似的素材，自动打语义标签
- ⛓️ **自动流水线**：分割 → 混剪 → 发布串成管线，一键或定时自动执行，失败即停
...
### AI 切片剪辑
长视频 → 镜头检测 → AI 精彩度评估 → 自动拆分为多条独立短视频，每条可一键进入**文本精剪**继续打磨。

### 文本精剪
转写文本与时间线双向绑定：点句子跳画面、选中删句、一键清理口头禅 / 压缩停顿、💬 对话式改片（AI 生成可勾选的编辑计划）。所有编辑落在非破坏性 EDL，导出前自动校验一致性，原素材永不改动。

### 热点选题
聚合百度 / 微博 / 抖音热榜，LLM 结合素材库方向生成 5 条选题建议，一键生成口播脚本并落盘。
