...
describe('model-downloader', () => {
  ...
  describe('路径辅助函数', () => {
    it('getClipModelDir 返回注入的临时目录', () => {
      assert.equal(getClipModelDir(), tempDir);
    });

    it('三个权重路径均落在模型目录下且文件名正确', () => {
      assert.equal(basename(getImageModelPath()), CN_IMAGE_MODEL_FILENAME);
      assert.equal(basename(getTextModelPath()), CN_TEXT_MODEL_FILENAME);
      assert.equal(basename(getVocabPath()), CN_VOCAB_FILENAME);
      for (const p of [getImageModelPath(), getTextModelPath(), getVocabPath()]) {
        assert.equal(join(tempDir, basename(p)), p);
      }
      assert.ok(getImageModelPath().startsWith(tempDir));
      assert.ok(getTextModelPath().startsWith(tempDir));
      assert.ok(getVocabPath().startsWith(tempDir));
    });
  });
