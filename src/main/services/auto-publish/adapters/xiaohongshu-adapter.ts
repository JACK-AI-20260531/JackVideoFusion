 ...
 * 小红书创作者平台适配器
 *
 * 职责:实现小红书创作者中心的登录、登录检测、登出与视频发布流程
 *
 * 平台特性:
 *   - 登录/上传页:https://creator.xiaohongshu.com/publish/publish
 *   - 标题输入:input
 *   - 发布:点击「发布」按钮
 *
 * 注意:小红书页面结构可能随平台更新变化,选择器采用多候选回退策略。
 */
...
export class XiaohongshuAdapter extends BasePlatformAdapter {
  /** 构造小红书适配器,platform 固定为 'xiaohongshu' */
  constructor() {
    super('xiaohongshu');
  }
  ...
  protected getLoggedInIndicator(): string {
    return '[class*="user"], [class*="avatar"], [class*="nickname"], [class*="account"]';
  }
  ...
  protected async doUpload(page: Page, params: PublishParams): Promise<void> {
    ...
    if (!ok) {
      throw new Error('小红书:未找到视频上传入口(input[type=file])');
    }
    ...
    if (!done) {
      throw new Error('小红书:视频上传超时,未检测到发布按钮');
    }
  }
