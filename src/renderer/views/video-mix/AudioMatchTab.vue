...
/**
 * 文件夹音频匹配 Tab(模式二)
 * 职责:每文件夹独立抽取 1 条音频 + N 条视频 → 合成片段 → 拼接为最终视频
 * 文件夹隔离:每个 folderId 独立产出片段,绝不在 matcher 里跨文件夹抽取
 * 调用 IPC:video-mix:start(mode='audio-match')、video-mix:pause、video-mix:cancel
...
// 是否去除原视频声音
const stripOriginalAudio = ref(true);
// 背景音乐淡入淡出(秒)
const audioFadeSec = ref(0.5);
// 音频是否循环适配视频时长
const audioLoop = ref(true);
// 分辨率预设
...
async function handleStart(): Promise<void> {
  ...
  const params: MixParams = {
    ...
    stripOriginalAudio: stripOriginalAudio.value,
    audioFadeSec: audioFadeSec.value,
    audioLoop: audioLoop.value,
    ...
    outputName: outputName.value || `audio-mix-${Date.now()}.mp4`,
  };
...
<template>
  <div class="mix-tab">
    ...
    <!-- 音频匹配参数 -->
    <section class="form-section">
      <h3 class="section-title">音频匹配参数</h3>
      <div class="form-row">
        <label class="form-label">每文件夹视频数</label>
        <input v-model.number="perFolderCount" type="number" min="1" class="form-input form-input--narrow" />
        <span class="form-hint">条(每文件夹抽 1 条音频 + N 条视频)</span>
      </div>
      <div class="form-row">
        <label class="form-label">音频淡入淡出</label>
        <input v-model.number="audioFadeSec" type="number" min="0" step="0.1" class="form-input form-input--narrow" />
        <span class="form-hint">秒(0=无淡入淡出)</span>
      </div>
      <div class="form-row form-row--inline">
        <label class="form-checkbox">
          <input v-model="stripOriginalAudio" type="checkbox" /> 去除原视频声音
        </label>
        <label class="form-checkbox">
          <input v-model="audioLoop" type="checkbox" /> 音频循环适配视频时长
        </label>
      </div>
    </section>
